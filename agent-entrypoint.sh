...
# All other providers (groq, mistral, xai, etc.) are accessed through OpenRouter.
...
openclaw cron add \\
  ...
  --session isolated \\
  --message "<same prompt>" \\
  --announce --channel telegram --to "<current chat id>"
\`\`\`

For \`--to\`, use the chat ID of the conversation you are currently in. The Telegram channel plugin handles delivery natively. If you don't know the chat ID, omit \`--announce --channel --to\` and the job will still run, but won't push a message back automatically.
...
echo "DEBUG [keys] OPENAI_API_KEY=$([ -n "${OPENAI_API_KEY}" ] && echo 'SET' || echo 'UNSET')"
...
# Check if the chosen provider's key is present; if not, fall back
_need_fallback=false
case "${AGENT_PROVIDER}" in
  anthropic)   [ -z "${ANTHROPIC_API_KEY}" ]   && _need_fallback=true ;;
  google)      [ -z "${GEMINI_API_KEY}" ]      && _need_fallback=true ;;
  openai)      [ -z "${OPENAI_API_KEY}" ]      && _need_fallback=true ;;
  openrouter)  [ -z "${OPENROUTER_API_KEY}" ]  && _need_fallback=true ;;
  *)           _need_fallback=true ;;
esac

if [ "${_need_fallback}" = "true" ]; then
  echo "WARNING: Key missing for provider ${AGENT_PROVIDER}"
  if [ -n "${OPENROUTER_API_KEY}" ]; then
    ...
    EFFECTIVE_AGENT_MODEL="openai/gpt-4o-mini"
  ...
  echo "WARNING: Falling back to ${EFFECTIVE_PROVIDER}: ${EFFECTIVE_AGENT_MODEL}"
fi
...
for t in tasks:
    ...
    args = ["openclaw", "cron", "add",
            ...
            "--session", session,
            "--message", prompt]
    # Native channel delivery via the @openclaw/telegram plugin. Discord/
    # WhatsApp not yet wired for cron delivery; jobs still run and the
    # output stays in OpenClaw's session transcript.
    if channel == "telegram" and to:
        args += ["--announce", "--channel", "telegram", "--to", str(to)]
    else:
        args += ["--no-deliver"]
