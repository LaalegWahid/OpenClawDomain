...
const C = {
  ...
  card:        '#f0e8de',
  cardAlt:     '#ebe0d4',
  border:      'rgba(42,31,25,0.12)',
  borderSoft:  'rgba(42,31,25,0.07)',
  text:        '#2a1f19',
  muted:       '#8a7060',
  faint:       '#a89080',
  accent:      '#FF4D00',
  accentDim:   'rgba(255,77,0,0.08)',
  accentBorder:'rgba(255,77,0,0.18)',
  codeBg:      '#e8ddd4',
  codeText:    '#4a3a30',
  codeAlt:     '#d8cec4',
}
...
function Code({ children }: { children: React.ReactNode }) {
  return (
    <code style={{
      ...
      background: C.codeBg, border: `0.5px solid ${C.border}`,
      color: C.accent, padding: '2px 7px', borderRadius: '5px',
    }}>
      {children}
    </code>
  )
...
function CodeBlock({ children, title }: { children: string; title?: string }) {
  ...
  return (
    <div style={{
      ...
      border: `0.5px solid ${C.border}`, margin: '1.25rem 0',
    }}>
      {title && (
        <div style={{
          background: C.codeAlt, borderBottom: `0.5px solid ${C.border}`,
          padding: '8px 16px', display: 'flex', justifyContent: 'space-between', alignItems: 'center',
        }}>
          <span style={{ fontSize: '11px', color: C.muted, fontFamily: 'var(--mono, monospace)' }}>{title}</span>
          <button
            onClick={() => { navigator.clipboard.writeText(children); setCopied(true); setTimeout(() => setCopied(false), 1500) }}
            style={{
              background: 'none', border: 'none', cursor: 'pointer',
              fontSize: '11px', color: copied ? C.accent : C.faint,
      ...
      <pre style={{
        background: C.codeBg, margin: 0, padding: '1.25rem 1.5rem',
        fontSize: '13px', color: C.codeText, lineHeight: 1.8,
        overflowX: 'auto', fontFamily: 'var(--mono, monospace)',
      }}>
        {children}
      </pre>
    </div>
...
function Callout({ type = 'info', children }: { type?: 'info' | 'warning' | 'tip'; children: React.ReactNode }) {
  const colors = {
    info:    { border: '#3B82F6', bg: '#3B82F610', icon: 'ℹ', text: '#1D4ED8' },
    warning: { border: '#D97706', bg: '#D9770610', icon: '⚠', text: '#B45309' },
    tip:     { border: C.accent,  bg: C.accentDim, icon: '◆', text: C.accent  },
  }
  const c = colors[type]
  return (
    <div style={{
...
function Section({ id, children }: { id: string; children: React.ReactNode }) {
  return (
    <section id={id} style={{ padding: '3rem 0', borderBottom: `0.5px solid ${C.border}` }}>
      {children}
    </section>
  )
}
...
function H1({ children }: { children: React.ReactNode }) {
  return (
    <h1 style={{
      fontFamily: 'var(--serif)', fontSize: 'clamp(1.8rem, 3vw, 2.6rem)', fontWeight: 500,
      letterSpacing: '-0.03em', color: C.text, marginBottom: '0.5rem', lineHeight: 1.2,
    }}>
      {children}
    </h1>
  )
...
function H3({ children }: { children: React.ReactNode }) {
  return (
    <h3 style={{
      ...
      color: C.text, marginTop: '1.75rem', marginBottom: '0.5rem',
    }}>
      {children}
    </h3>
  )
...
function P({ children }: { children: React.ReactNode }) {
  return (
    <p style={{ fontSize: '14px', color: C.muted, lineHeight: 1.9, margin: '0.75rem 0' }}>
      {children}
    </p>
  )
}
  ...
  return (
    <div style={{ overflowX: 'auto', margin: '1.25rem 0' }}>
      <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '13px' }}>
          ...
          <tr>
            {headers.map(h => (
              <th key={h} style={{
                ...
                borderBottom: `0.5px solid ${C.border}`, background: C.cardAlt,
              }}>
                {h}
              </th>
            ))}
        ...
        <tbody>
          {rows.map((row, i) => (
            <tr key={i} style={{ borderBottom: `0.5px solid ${C.borderSoft}` }}>
              {row.map((cell, j) => (
                <td key={j} style={{
                  padding: '10px 14px', color: j === 0 ? C.text : C.muted,
    ...
    <>
      <style>{`
        * { box-sizing: border-box; }
        ...
        ::-webkit-scrollbar-thumb { background: rgba(42,31,25,0.2); border-radius: 4px; }
        .doc-nav-link { transition: color 0.15s, background 0.15s; }
        .doc-nav-link:hover { color: ${C.text} !important; background: rgba(42,31,25,0.06) !important; }
        .doc-mobile-bar { display: none; }
        /* desktop */
        @media(min-width: 861px) {
          .doc-layout { padding-top: 60px; }
          .doc-main   { margin-left: 220px; padding: 3rem 3rem 6rem; }
        }
        /* mobile */
        @media(max-width: 860px) {
          .doc-sidebar    { display: none !important; }
          .doc-layout     { padding-top: 108px; }
          .doc-main       { margin-left: 0 !important; padding: 1.75rem 1.25rem 5rem !important; }
          .doc-mobile-bar {
            display: flex;
            position: fixed; top: 60px; left: 0; right: 0; height: 48px;
            background: ${C.card}; border-bottom: 0.5px solid ${C.border};
            align-items: center; justify-content: space-between;
            padding: 0 1.25rem; z-index: 50;
          }
          .doc-overlay { top: 108px !important; }
        }
      `}</style>
      ...
      <div style={{ background: C.bg, color: C.text, minHeight: '100vh', fontFamily: 'system-ui, -apple-system, sans-serif' }}>
        ...
        {/* Mobile nav bar — hamburger strip below Navbar */}
        <div className="doc-mobile-bar">
          <span style={{ fontSize: '12px', fontWeight: 600, color: C.muted, letterSpacing: '0.08em', textTransform: 'uppercase' }}>
            Documentation
          </span>
          <button
        ...
        {mobileNavOpen && (
          <div
            ...
            style={{
              ...
              background: `rgba(248,242,237,0.98)`, zIndex: 49,
            ...
            {NAV.map(item => (
              <button
                ...
                style={{
                  ...
                  background: activeSection === item.id ? C.accentDim : 'none',
                  border: 'none', cursor: 'pointer',
                  padding: '12px 14px', borderRadius: '8px',
                  fontSize: '15px', color: activeSection === item.id ? C.accent : C.muted,
        ...
        <div className="doc-layout" style={{ display: 'flex' }}>
          ...
          <aside className="doc-sidebar" style={{
            ...
            <div style={{ padding: '0 1rem' }}>
              <div style={{
                ...
                color: C.faint, fontWeight: 600, marginBottom: '0.75rem', padding: '0 8px',
              }}>
                Documentation
              </div>
              {NAV.map(item => (
                <button
                  ...
                  style={{
                    display: 'block', width: '100%', textAlign: 'left',
                    background: activeSection === item.id ? C.accentDim : 'transparent',
                    border: 'none', cursor: 'pointer',
          ...
          >
            ...
            <Section id="introduction">
              ...
              <p style={{ fontSize: '1.05rem', color: C.muted, lineHeight: 1.8, margin: '1rem 0 2rem' }}>
              ...
              <div style={{
                ...
                {[
                  {
                    icon: (
                      <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke={C.accent} strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round">
                  ...
                  <div key={c.title} style={{
                    ...
                    <div style={{ fontSize: '13px', fontWeight: 600, color: C.text, marginBottom: '4px' }}>{c.title}</div>
                    <div style={{ fontSize: '12px', color: C.muted, lineHeight: 1.6 }}>{c.desc}</div>
                  </div>
                ))}
              </div>
            ...
            <Section id="quickstart">
              ...
              {[
                { step: '01', title: 'Create your account', desc: 'Sign up at openclaw.sltverse.com. No credit card required to get started.' },
                { step: '02', title: 'Start your free trial', desc: 'Every account gets a 15-day free trial on the first agent. No payment info needed.' },
                { step: '03', title: 'Build your agent', desc: 'Name your agent, write its system prompt, and pick the domain it operates in.' },
                { step: '04', title: 'Connect a messaging platform', desc: 'Add a Telegram bot token, a Discord bot token, or scan a WhatsApp QR code.' },
                { step: '05', title: 'Deploy', desc: 'Hit Deploy. Your agent spins up in seconds and is live in your chat.' },
              ].map(s => (
                <div key={s.step} style={{
                  ...
                  <div>
                    <div style={{ fontSize: '14px', fontWeight: 600, color: C.text, marginBottom: '4px' }}>{s.title}</div>
                    <div style={{ fontSize: '13px', color: C.muted, lineHeight: 1.7 }}>{s.desc}</div>
                  </div>
                </div>
              ))}
...
→ Your agent replies with a summary of recent activity in its domain.`}</CodeBlock>
            ...
            <Section id="agents">
              ...
              <P>From the dashboard, click <strong style={{ color: C.text }}>New Agent</strong>. You'll configure:</P>
              <Table
            ...
            <Section id="skills">
              ...
              <H1>Skills</H1>
              <P>Skills are the capabilities you give an agent. They tell the agent what it knows how to do, how to structure its outputs, and which tools or workflows to invoke when asked.</P>
              ...
              <P>From the agent's <strong style={{ color: C.text }}>Skills</strong> tab in the dashboard, you can browse and import pre-built skills:</P>
              <div style={{
                ...
                {[
                  ...
                  <div key={s.name} style={{
                    ...
                    <div style={{ fontSize: '13px', fontWeight: 600, color: C.text, marginBottom: '4px' }}>{s.name}</div>
                    <div style={{ fontSize: '12px', color: C.muted, lineHeight: 1.6 }}>{s.desc}</div>
                  </div>
                ))}
              </div>

              <H2>Custom skill builder</H2>
              <P>For skills that don't exist in the library, write them yourself in plain language. Define:</P>
...
Trigger phrases: due diligence, DD report, risk assessment`}</CodeBlock>
              <P>Once saved, the agent learns this skill and will invoke it whenever the trigger context matches.</P>
...
uvx    — Python tools via uv`}</CodeBlock>
              ...
              <P>From the agent's <strong style={{ color: C.text }}>MCP Servers</strong> tab:</P>
...
Headers:   { "Authorization": "Bearer your-token" }`}</CodeBlock>
            ...
            <Section id="integrations">
              ...
              <H1>Integrations</H1>
              <P>
                Your agents live where your team already is. OpenClaw supports three messaging platforms. Each is configured separately and can be added to any agent at any time from the <strong style={{ color: C.text }}>Channels</strong> tab.
              </P>

              {/* Telegram */}
              <div style={{ border: `0.5px solid ${C.border}`, borderRadius: '14px', overflow: 'hidden', margin: '2rem 0' }}>
                <div style={{
                  background: 'rgba(42,160,230,0.08)', padding: '1.25rem 1.5rem',
                  borderBottom: `0.5px solid ${C.border}`,
                  display: 'flex', alignItems: 'center', gap: '10px',
                }}>
                  <img src="/images/telegram.png" alt="Telegram" style={{ width: '32px', height: '32px', objectFit: 'contain' }} />
                  <div>
                    <div style={{ fontSize: '14px', fontWeight: 600, color: C.text }}>Telegram</div>
                    <div style={{ fontSize: '11px', color: '#1A8FD1' }}>Webhook-based · Instant delivery</div>
                  </div>
...
Paste this token into OpenClaw when creating your agent.`}</CodeBlock>
              ...
              <div style={{ border: `0.5px solid ${C.border}`, borderRadius: '14px', overflow: 'hidden', margin: '2rem 0' }}>
                <div style={{
                  background: 'rgba(88,101,242,0.08)', padding: '1.25rem 1.5rem',
                  borderBottom: `0.5px solid ${C.border}`,
                  display: 'flex', alignItems: 'center', gap: '10px',
                }}>
                  <img src="/images/discord.png" alt="Discord" style={{ width: '32px', height: '32px', objectFit: 'contain' }} />
                  <div>
                    <div style={{ fontSize: '14px', fontWeight: 600, color: C.text }}>Discord</div>
                    <div style={{ fontSize: '11px', color: '#5865F2' }}>Bot gateway · Server channels</div>
                  </div>
                </div>
                <div style={{ padding: '1.5rem' }}>
                  <H3>Setup</H3>
                  <P>
                    Create a bot application at <Code>discord.com/developers</Code>. Under <strong style={{ color: C.text }}>Bot</strong>, enable <Code>Message Content Intent</Code> and copy the bot token. Invite the bot to your server with the <Code>bot</Code> scope and <Code>Send Messages</Code> permission.
                  </P>
              ...
              <div style={{ border: `0.5px solid ${C.border}`, borderRadius: '14px', overflow: 'hidden', margin: '2rem 0' }}>
                <div style={{
                  background: 'rgba(37,211,102,0.08)', padding: '1.25rem 1.5rem',
                  borderBottom: `0.5px solid ${C.border}`,
                  display: 'flex', alignItems: 'center', gap: '10px',
                }}>
                  <img src="/images/whatsapp.png" alt="WhatsApp" style={{ width: '32px', height: '32px', objectFit: 'contain' }} />
                  <div>
                    <div style={{ fontSize: '14px', fontWeight: 600, color: C.text }}>WhatsApp</div>
            ...
            <Section id="byok">
              ...
              <div style={{
                ...
                {[
                  ...
                  <div key={c.title} style={{
                    background: C.card, border: `0.5px solid ${C.accentBorder}`,
                    borderRadius: '10px', padding: '1rem',
                  }}>
                    <div style={{ fontSize: '13px', fontWeight: 600, color: C.text, marginBottom: '4px' }}>{c.title}</div>
                    <div style={{ fontSize: '12px', color: C.muted, lineHeight: 1.6 }}>{c.desc}</div>
              ...
              <P>OpenClaw agents are currently powered by <strong style={{ color: C.text }}>Google Gemini 2.5 Flash</strong> — optimised for real-time, high-throughput responses with a large context window. Support for additional providers (OpenAI, Anthropic) is on the roadmap.</P>

              <H2>Configuring your key</H2>
              <P>
                Add your API key during agent creation or from <strong style={{ color: C.text }}>Settings → API Keys</strong>. The key is encrypted at rest and injected into your agent's environment at runtime. It is never logged or exposed in the dashboard.
              </P>
              <Callout type="tip">
            ...
            <Section id="memory">
              ...
              <P>
                From the agent's <strong style={{ color: C.text }}>Memory</strong> tab in the dashboard, you can view all active sessions with message counts and estimated token usage. You can clear all history for the agent with one click — this wipes all sessions and the agent starts fresh on the next message.
              </P>
              <Callout type="warning">
            ...
            <Section id="documents">
              ...
              <H1>PDF Documents</H1>
              <P>Any agent can generate a professionally formatted PDF and deliver it directly in your chat. No third-party tools, no browser — just ask.</P>
...
"I need a report on..."`}</CodeBlock>
              ...
              <H2>Delivery</H2>
              <P>The PDF is delivered as a file attachment directly in the chat. On Telegram, it appears as a document you can save or forward. On Discord and WhatsApp, it appears as a downloadable file attachment.</P>
              <Callout type="tip">
            ...
            <Section id="dashboard">
              ...
              <H1>Dashboard</H1>
              <P>The OpenClaw dashboard is your control plane. Everything about your agents — configuration, channels, skills, memory, logs, and billing — lives here.</P>

              <H2>Overview</H2>
              <P>The main overview screen shows all your deployed agents with their current status, domain, and connected platforms. From here you can start, stop, or delete any agent, and create new ones.</P>
              ...
              <H2>Settings</H2>
              <P>Under <strong style={{ color: C.text }}>Settings</strong> you can update your profile, change your password, manage API keys, and delete your account.</P>

              <H2>Billing</H2>
              <P>Under <strong style={{ color: C.text }}>Settings → Billing</strong> you can view your current plan, manage payment methods, update your card, and cancel or resume your subscription. Billing is handled by Stripe — your card details are never stored on OpenClaw servers.</P>
            </Section>
            ...
            <Section id="pricing">
              ...
              <H1>Pricing</H1>
              <P>OpenClaw charges per agent per month. No flat platform fee. You only pay for the agents you have running — scale up or down at any time.</P>
              ...
              <div style={{
                ...
                {[
                  ...
                  <div key={t.name} style={{
                    background: C.card,
                    border: t.highlight ? `1px solid ${C.accentBorder}` : `0.5px solid ${C.border}`,
                    ...
                    <div style={{ fontSize: '12px', color: t.highlight ? C.accent : C.muted, fontWeight: 500, marginBottom: '8px' }}>{t.name}</div>
                    <div style={{ fontSize: '2rem', fontWeight: 500, letterSpacing: '-0.04em', color: C.text, lineHeight: 1, fontFamily: 'var(--serif)' }}>{t.price}</div>
                    <div style={{ fontSize: '11px', color: C.muted, marginBottom: '4px' }}>{t.unit}</div>
                    <div style={{ fontSize: '12px', color: C.muted, marginBottom: '1rem', paddingBottom: '1rem', borderBottom: `0.5px solid ${C.border}` }}>{t.agents}</div>
                    {t.features.map(f => (
                      <div key={f} style={{ display: 'flex', gap: '7px', alignItems: 'flex-start', marginBottom: '7px' }}>
                        <span style={{ color: C.accent, fontSize: '11px', flexShrink: 0, marginTop: '2px' }}>✓</span>
                        <span style={{ fontSize: '12px', color: C.muted, lineHeight: 1.5 }}>{f}</span>
                      </div>
                    ))}
                  </div>
                ))}
            ...
            <Section id="security">
              ...
              <P>Every agent runs in its own dedicated AWS ECS Fargate container. Your agent's data, credentials, and conversation history never share memory or storage with any other user's agent. Container boundaries are enforced at the infrastructure level.</P>
              ...
              <H2>Domain boundary enforcement</H2>
              <P>Agent system prompts include a hard-coded boundary preamble that cannot be overridden by user messages. This prevents agents from being repurposed outside their declared domain and blocks prompt injection attempts at the model instruction level.</P>

              <H2>Data deletion</H2>
              <P>You can delete your account and all associated data at any time from <strong style={{ color: C.text }}>Settings → Account</strong>. This removes your profile, all agents, all conversation history, and all billing records from our systems within 30 days.</P>

              <H2>Admin kill-switch</H2>
              <P>OpenClaw has a global service kill-switch accessible to platform administrators. When enabled, all agent message processing stops immediately — no tokens are consumed and no responses are sent. This is used only in exceptional operational circumstances.</P>
            </Section>
            ...
            <Section id="faq">
              ...
              {[
                { q: 'Can I build an agent for any industry?', a: 'Yes. OpenClaw is not limited to preset domains. You write the system prompt, define the domain, and build custom skills for your exact workflows. Legal, healthcare, real estate, logistics, education, HR — if you can describe the work, your agent can handle it.' },
                { q: 'What AI model do the agents use?', a: 'Agents are powered by Google Gemini 2.5 Flash — optimised for fast, real-time responses with a large context window. With BYOK, you connect your own Google AI API key and your agents use it directly.' },
                { q: 'Can one agent be on multiple platforms at the same time?', a: 'Yes. A single agent can be connected to Telegram, Discord, and WhatsApp simultaneously. Each platform connection is a separate channel. Each platform maintains its own independent conversation history per chat.' },
                { q: 'What happens if my agent goes down?', a: 'OpenClaw monitors all containers continuously. If a container stops unexpectedly, it is automatically restarted. You can also manually restart from the dashboard. Unexpected stops are logged in the activity timeline.' },
                { q: 'Is conversation history stored permanently?', a: 'Conversation history is stored in your dedicated database slot and scoped to your agent and chat ID. You can view, estimate token usage, and clear history at any time from the Memory tab. Clearing is permanent.' },
                { q: 'Does OpenClaw see my API key?', a: 'Your BYOK key is injected directly into your agent\'s container environment at launch. It is never logged, never returned by the API, and never visible in the dashboard after initial entry. It is used exclusively to make AI calls on your behalf.' },
                { q: 'Can I add multiple MCP servers to one agent?', a: 'Yes. An agent can have multiple MCP servers active at the same time. Each server exposes its own set of tools. The agent selects relevant tools automatically at runtime based on the user\'s message.' },
                { q: 'How do I cancel my subscription?', a: 'Cancel from Settings → Billing with one click. No calls, no forms. Your agents continue running until the end of the billing period, then they stop. You can reactivate at any time.' },
                { q: 'What is the 15-day free trial?', a: 'Every new account gets 15 days free on the first agent — no credit card required. After 15 days, you\'ll be prompted to subscribe to keep the agent running. Additional agents require a subscription from day one.' },
                { q: 'What is the uptime SLA?', a: 'Tier C includes a 99.9% uptime SLA. OpenClaw monitors all containers and the platform infrastructure 24/7. Scheduled maintenance is communicated in advance.' },
              ].map((item, i) => (
                <div key={i} style={{ borderBottom: `0.5px solid ${C.borderSoft}`, padding: '1.25rem 0' }}>
                  <div style={{ fontSize: '14px', fontWeight: 500, color: C.text, marginBottom: '0.5rem', fontFamily: 'var(--serif)' }}>
                    {item.q}
                  </div>
              ...
              <div style={{
                ...
                <div style={{ fontSize: '1.1rem', fontWeight: 500, color: C.text, marginBottom: '0.5rem', fontFamily: 'var(--serif)' }}>
