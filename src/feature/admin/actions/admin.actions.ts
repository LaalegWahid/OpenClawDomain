...
export async function getAdminDashboardData() {
  ...
  const userGrowth =
    (userGrowthRows as unknown as { rows?: Array<{ day: string; count: number }> }).rows ??
    (userGrowthRows as unknown as Array<{ day: string; count: number }>);
  const agentActivity =
    (agentActivityRows as unknown as { rows?: Array<{ day: string; count: number }> }).rows ??
  ...
  return {
    ...
    stats: { totalUsers, totalAgents, activeAgents, adminCount },
